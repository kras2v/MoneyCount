...
const Statistics = () => {
	...
	const [outcomesOnRangeData, setOutcomesOnRangeData] = useState(null);
	...
	const askAI = async () => {
		...
		setAnswer("");
		try {
			const outcomeOnRangeDataJson = JSON.stringify(outcomesOnRangeData.reverse());
			const incomeDataJson = JSON.stringify(incomesData.reverse());
			const genAI = new GoogleGenerativeAI("AIzaSyAH5-nJiYVADHFE7LFkY37HK2bzhZAYqrQ");
			const model = genAI.getGenerativeModel({ model: "gemini-1.5-flash" });
			const prompt = `You are an AI support assistant for a budgeting application.
			Your task is to provide **three brief suggestions** on how the user can improve their budget.
			- The user has been a member since **${userInfo.createdDate.split("T")[0]}**.
			- Their spending details for **${outcomePeriodLabel}** are as follows:
			${outcomeOnRangeDataJson}
			- Their monthly income for the last 6 months:
			${incomeDataJson}
			Please analyze the data and give **concise, practical, and actionable** advice.`;
			const result = await model.generateContent(prompt);
			setAnswer(result.response.text());
		}
		catch (err) {
			setAnswer("Error fetching AI response. Please try again." + err);
		}
		finally {
			setLoading(false);
		}
	...
	useEffect(() => {
		...
		if (categories && outcomesOnDateRange && incomesOverSixMonths) {
			...
			setOutcomeChartData(createOutcomeChart(categories, outcomesOnDateRange, setOutcomesOnRangeData, 'outcome-chart'));
	...
	return (
		<div className="custom-container p-4">
			{userInfo ? (<div className="d-flex flex-column align-items-center gap-4">
				<div className="outcome-statistiscs d-flex flex-column align-items-center w-100 p-4 border rounded-lg shadow-lg bg-gray-50 max-w-md mx-auto">
					...
					<div className="d-flex mt-4 justify-content-between gap-3 w-100">
						<div className="w-25 chart-container x justify-content-center mt-4 d-flex align-items-center" style={{ minWidth: "50%", minHeight: "50%" }}>
